...
- **Verdict:** PARTIAL
- **Classification:** One reusable check is consistently called today, but it is a runtime single point with test-only secondary detection
- **Where:** The single effective tenant check is `require_patient_access`, specifically `user.clinic_id != patient.clinic_id`, in `backend/app/services/authorization_service.py:29-34`. Routes first resolve an object to its patient, then call this function. Service getters such as `get_patient`, `get_entry`, `get_comment`, and `get_assignment` are not independently clinic-scoped.
- **What breaks first:** If that clinic-comparison line accepts the wrong clinic, a staff, clinician, or admin from Clinic B who knows or guesses a Clinic A patient/entry/comment/assignment/highlight/conflict ID can retrieve or mutate every Clinic A record reachable through the current ID-based routes. There is no patient-list endpoint, so all patients do not automatically appear in one screen, but the blast radius is all addressable patients and linked records, not one note.
- **What could break later:** `can_view_entry` still limits information by role and entry type, and note ownership still limits some edits, but neither is tenant isolation. Database foreign keys preserve relationships, not clinic ownership. The only existing second line is regression detection: cross-clinic tests would fail when run; they do not block a bad production request.
- **Improvement:** Phase 2 should add tenant-aware repository/query methods that require `clinic_id`, model authenticated clinic membership, add defense-in-depth query predicates/constraints where practical, and parameterize every linked-entity read/write test. The route check should remain as an outer guard rather than being removed.
- **Evidence:** `test_rbac_scope.py::test_staff_and_clinician_cannot_access_other_clinic`, `test_ai_scribe.py::test_ai_scribe_enforces_clinic_scope`, conflict and approval clinic-scope tests. There is no exhaustive cross-entity test and no independent runtime tenant boundary.
- **Remaining limitation:** Development headers are forgeable and SQLite provides no production row-level security; even two application checks would not make this production multi-tenancy.
...
- **Verdict:** PARTIAL
- **Classification:** A second clinic ID can be seeded, but there is no supported onboarding or production multi-clinic operating model
- **Where:** `Patient.clinic_id` and clinic-scoped importance preferences exist (`backend/app/models/patient.py:12-14`; `backend/app/models/importance_preference.py:8-16`), and `clinic-demo-002` is seeded (`backend/app/services/seed.py:79-87`). There is no Clinic model, User/ClinicMembership model, migration framework, provisioning command, clinic administration UI, or deployment configuration.
...
- **Improvement:** Required changes are explicitly separated: **config**—clinic-specific names, allowed origins, provider/channel settings, feature flags and secrets; **schema**—Clinic and User/ClinicMembership tables, explicit tenant constraints/indexes and migration tooling; **identity/membership**—replace asserted headers with authenticated users and verified clinic-role membership; **data migration**—create Clinic A/B records, backfill and validate tenant ownership for patients and linked data, remove startup seed overwrite behavior, and verify isolation; **deployment**—run migrations, move to managed production storage, configure TLS/encryption/secrets/backups/monitoring, disable demo identities/seeding, and conduct tenant-isolation smoke tests.
- **Evidence:** Current second-clinic seed plus `test_rbac_scope.py::test_staff_and_clinician_cannot_access_other_clinic`; no onboarding, membership, migration, or deployment test exists.
- **Remaining limitation:** This Phase 0 answer is a change inventory, not proof that Clinic B can safely launch next Monday.
